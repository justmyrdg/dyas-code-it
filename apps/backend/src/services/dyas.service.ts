...
export interface DyasContext {
  ...
  // Human-readable description of what the student is looking at, e.g. the
  // lesson title + an excerpt, or the activity prompt. Empty for 'general'.
  summary: string;
}
...
export function buildSystemPrompt(context: DyasContext): string {
...
export function toClaudeMessages(history: DyasMessage[]): { role: 'user' | 'assistant'; content: string }[] {
...
class ClaudeDyasClient implements DyasClient {
  async reply(history: DyasMessage[], context: DyasContext): Promise<string> {
    ...
    const blocks = (response.data as { content?: { type: string; text?: string }[] }).content ?? [];
    return blocks
...
export const dyasClient: DyasClient = new ClaudeDyasClient();
